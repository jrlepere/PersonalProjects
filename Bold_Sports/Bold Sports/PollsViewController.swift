 ...
 */
class PollsViewController: UIViewController, KumulosDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UIScrollViewDelegate {
    ...
    var usernameOfFriendRequested = ""
    var predictionUserID:UInt = 0
    let addFriendDimensionsRatio: CGFloat = 0.15
    ...
    func createPredictionView(result: AnyObject, index: Int) {
        ...
        // Add Friend Button
        let addFriendButton = AddFriendButton(type: .System)
        addFriendButton.predictionCreatorPollID = predictionData.userPollID
        addFriendButton.username = predictionData.username
        let addFriendSideLength = scrollViewWidth*addFriendDimensionsRatio
        addFriendButton.frame = CGRect(x: scrollViewWidth - addFriendSideLength, y: 0, width: addFriendSideLength, height: addFriendSideLength)
        addFriendButton.setTitle("+", forState: .Normal)
        addFriendButton.setTitleColor(UIColor.blueColor(), forState: .Normal)
        addFriendButton.layer.borderWidth = 0.2
        addFriendButton.layer.borderColor = UIColor.blackColor().CGColor
        addFriendButton.titleLabel?.font = UIFont(name: (addFriendButton.titleLabel?.font?.fontName)!, size: 30)
        addFriendButton.titleLabel?.adjustsFontSizeToFitWidth = true
        addFriendButton.addTarget(self, action: #selector(PollsViewController.addFriendPressed(_:)), forControlEvents: .TouchUpInside)
        ...
        if (predictionData.username != (self.tabBarController as! MainTabBarController).userInfo.username) {
            predictionView.addSubview(addFriendButton)
        }
    ...
    /**
     Preforms when the user taps the add friend button.
     */
    func addFriendPressed(sender: AddFriendButton) {
        
        ProcessingIndicator.startProcessingIndicator(processingIndicator, viewsToHide: [sportPicker])
        
        predictionUserID = UInt(sender.predictionCreatorPollID)
        usernameOfFriendRequested = sender.username
        
        let sendFriendRequestAlert = UIAlertController(title: "Friend Request", message: "Would you like to send \(sender.username) a friend request?", preferredStyle: .Alert)
        sendFriendRequestAlert.addAction(UIAlertAction(title: "Yes", style: .Default, handler: { (UIAlertAction) in
            self.K.checkIfFriendsWithUsername((self.tabBarController as! MainTabBarController).userInfo.username, andFriend: UInt(sender.predictionCreatorPollID))
        }))
        sendFriendRequestAlert.addAction(UIAlertAction(title: "NO", style: .Default, handler: { (UIAlertAction) in
            ProcessingIndicator.stopProcessingIndicator(self.processingIndicator, viewsToUnHide: [self.sportPicker])
        }))
        self.presentViewController(sendFriendRequestAlert, animated: true, completion: nil)
    ...
    func scroll() {
        ...
        self.scrollView.scrollRectToVisible(CGRect(x: CGFloat(currentPredictionIndex) * self.scrollView.frame.width, y: 0, width: self.scrollViewWidth, height: self.scrollViewHeight - (self.navigationController?.navigationBar.frame.height)!-8), animated: true)
    ...
    func kumulosAPI(kumulos: Kumulos!, apiOperation operation: KSAPIOperation!, checkIfFriendsDidCompleteWithResult theResults: [AnyObject]!) {
        ...
        if theResults.count > 0 {
            ...
            if requested {
                // already friends
                let alreadyFriendsAlert = UIAlertController(title: "Already Friends", message: "You and \(usernameOfFriendRequested) are already friends!", preferredStyle: .Alert)
